...
EXPLICIT_IMAGE_REGEX = re.compile(
    ...
    r'a\s+tabela\s+(?:a\s+seguir|abaixo|apresenta|mostra|indica|ilustra|representa)|'
    r'na\s+tabela\s+(?:a\s+seguir|abaixo)|'
    r'as\s+tabelas\s+(?:a\s+seguir|abaixo|apresentam|mostram)|'
    r'conforme\s+a\s+tabela|'
    r'como\s+(?:mostra|ilustra)\s+a\s+tabela|'
    r'dados\s+da\s+tabela|'
    r'informad[ao]s?\s+na\s+tabela|'
    r'observad[ao]s?\s+na\s+tabela|'
    r'o\s+quadro\s+(?:a\s+seguir|abaixo|apresenta|mostra|indica|ilustra|representa)|'
    r'no\s+quadro\s+(?:a\s+seguir|abaixo)|'
    r'os\s+quadros\s+(?:a\s+seguir|abaixo|apresentam|mostram)|'
    r'conforme\s+o\s+quadro|'
    r'como\s+(?:mostra|ilustra)\s+o\s+quadro|'
    r'dados\s+do\s+quadro|'
    r'informad[ao]s?\s+no\s+quadro|'
    r'observad[ao]s?\s+no\s+quadro'
    r')\b',
    re.IGNORECASE
)
